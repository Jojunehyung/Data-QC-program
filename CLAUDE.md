...
1. 식별번호 우선순위: O > X > 기타 (같은 병록번호 내)
2. O없고 X만 있을 때: 최신 X가 메인, 나머지 X → 가족데이터
3. 생성ID: 성별 + 출생연월(4자리연도+2자리월) + 이름첫글자 + 접수일자(YYYYMMDD)
4. 출생연월 2자리 → 4자리: `YEAR_THRESHOLD`(24) 이하면 2000년대, 초과면 1900년대
5. R-ID: 미매칭·중복 시트에서 제거, 매칭성공 시트에만 포함
6. 병록번호: 항상 8자리 (7자리면 앞에 0)
7. 오류 알림: 데모 환경에서 실제 발송 없이 미리보기만 표시
8. 수집일지 자동 수정: 병록번호 기준 매칭 후 변경된 값만 덮어쓰기
