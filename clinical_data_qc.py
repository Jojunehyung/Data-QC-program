...
def run_build_master_db(progress: ProgressWindow = None):
    ...
    if not df_active.empty:
        def id_priority(x):
            v = clean_str(x).upper()
            return 1 if v.startswith('P') else (2 if v.startswith('F') else 3)
...
def run_extract_for_rid_request(progress: ProgressWindow = None):
    ...
    IRB·DRB 승인 후 외부 시스템(데이터추출시스템 등)에 R-ID를 요청하기 위한 파일을 생성합니다.
    """
    if progress:
        progress.update(0, "파일 선택 대기 중...")
...
def _build_match_map(df_raw: pd.DataFrame, col_map: dict) -> Tuple[dict, set]:
...
def run_rid_matching(progress: ProgressWindow = None):
    ...
    try:
        ...
        rid_map, dup_keys = _build_match_map(df_raw, col_map)
...
def run_send_error_notification(progress: ProgressWindow = None):
    ...
    col = COL_BCODE if system == '수집관리시스템' else COL_HOSP_NUM
    col_label = COL_BCODE if system == '수집관리시스템' else COL_HOSP_NUM
...
def _read_emr_fix(path: Path) -> pd.DataFrame:
    """데이터추출시스템 수정파일 읽기 — '데이터추출 결과(xxx)' 형식, R-ID 대신 병록번호"""
    try:
...
def run_update_collection_log(progress: ProgressWindow = None):
    ...
    - 수집관리시스템: 원본과 동일한 포맷의 수정파일
    - 데이터추출시스템: '데이터추출 결과(xxx)' 형식 (R-ID 대신 병록번호)
    """
    if progress:
    ...
    try:
        ...
        if system == '수집관리시스템':
            ...
            df_fix = _read_emr_fix(Path(fix_path))
